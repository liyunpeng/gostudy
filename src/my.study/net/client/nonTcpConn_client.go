...
func ClientOnlySendReceiveNonTCPConn(address string) {
	...
	conn, err := net.Dial("tcp",  address+":7890")
	if err != nil {
		log.Fatal(err)
	}
	...
	/*
		...
		routinez在刚开始就wg.add(1)
		然後defer wg.done, 保证在程序退出时释放信号
	*/
	go recvNetwork1(conn , &wg)
...
func recvNetwork1(c net.Conn, wg *sync.WaitGroup) {
	wg.Add(1)
